...
describe("src/shared/js/utils/getWKTGeom.js", () => {
    it("should return a feature with geometryType and POLYGON coordinates", () => {
        ...
        expect(WKTUtil.getWKTGeom(content, geometryType)).is.not.undefined;
        expect(WKTUtil.getWKTGeom(content, geometryType).getGeometry()).is.not.undefined;
        expect(WKTUtil.getWKTGeom(content, geometryType).getGeometry().getCoordinates()).to.deep.equal([[
