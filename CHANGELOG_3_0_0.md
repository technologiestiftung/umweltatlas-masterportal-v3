...
- new config.json parameter 'tree' added. Containes:
    ...
    - 'addLayerButton': if active is true, a button to add layers is shown. On first menu side only Layers configured in config.json with attribute 'showInLayerTree':true or "visibility": true are shown.
