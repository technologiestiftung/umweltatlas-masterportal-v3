...
|layerSequence|no|Number||Number to determine the sequence of selected layers in the layerTree and layerSelection. The layers are rendered in sequence based on the assigned value, e.g. layers with `layerSequence=1` will always be on top, etc. Does not work for Baselayers.||
