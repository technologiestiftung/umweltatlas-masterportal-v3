...
|showPageNumber|no|Boolean|false|If set to true, displays the current position and total count of GFI responses next to the layer title (e.g., "(2/5)"). The counter is only shown when more than one response is available.|false|
